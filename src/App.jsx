  ...
  return (
    <div className="app-main">
      ...
      <AnimatePresence mode="wait">
        ...
        {level === 5 && <Level8 key="l5" onNext={nextLevel} onPenalty={addPenalty} />}
        {level === 6 && <LovePuzzle key="l6" onNext={nextLevel} />}
        {level === 7 && <Level7 key="l7" onNext={nextLevel} onPenalty={addPenalty} />}
        {level === 8 && <Level9 key="l8" onNext={nextLevel} />}

        {/* Phase 3: Fun Challenges */}
        {level === 9 && <Level6 key="l9" onNext={nextLevel} onPenalty={addPenalty} onCheat={addCheat} />}
        {level === 10 && <Level4 key="l10" onNext={nextLevel} />}

        {/* Phase 4: Action & Celebration */}
        {level === 11 && <Level10 key="l11" onNext={nextLevel} onPenalty={addPenalty} />}
        {level === 12 && <Level12 key="l12" onNext={nextLevel} />}
        {level === 13 && <Level13 key="l13" onNext={nextLevel} />}
        {level >= 14 && <FinalMessage key="final" chocolates={chocolates} cheated={cheated} />}
      </AnimatePresence>
    </div>
  );
}
